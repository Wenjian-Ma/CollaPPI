...
class Trainer(object):
    ...
    def train(self, dataset):
        ...
        for data in sampling:
            ...
            comb = (protein1.to(device), adjacency1.to(device), protein2.to(device), adjacency2.to(device),
...
class Tester(object):
    ...
    def test(self, dataset):
        ...
        tp, fp, tn, fn, accuracy, precision, sensitivity, recall, specificity, MCC, F1_score, Q9, ppv, npv = calculate_performace(
            len(sampling), labels, y_true)
    ...
    def result(self, epoch, time, loss, accuracy, precision, recall, sensitivity, specificity, MCC, F1_score,
               roc_auc_val, auc_val, Q9, ppv, npv, tp, fp, tn, fn, file_name):
        with open(file_name, 'a') as f:
            result = map(str, [epoch, time, loss, accuracy, precision, recall, sensitivity, specificity, MCC, F1_score,
                               roc_auc_val, auc_val, Q9, ppv, npv, tp, fp, tn, fn])
            f.write('\t'.join(result) + '\n')
...
examples_test = np.load(dir_input + 'one_to_one_test_examples.npy')
...
accuracy, precision, recall, sensitivity, specificity, MCC, F1_score, roc_auc_val, auc_val, Q9, ppv, npv, tp, fp, tn, fn = tester.test(dataset_test)
    ...
    # tester.result(epoch, time, loss, accuracy, precision, recall, sensitivity, specificity, MCC, F1_score, roc_auc_val,
    #               auc_val, Q9, ppv, npv, tp, fp, tn, fn, file_result)
